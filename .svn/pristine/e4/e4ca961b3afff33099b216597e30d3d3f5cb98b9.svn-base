...
/*$client = new SoapClient('http://qddemo.isaaviations.com/webservices/services/AAResWebServices?wsdl');
$result = $client->__getReservationbyPNR(array('pnrno' => 10000681, 'lastname' => 'adult'));*/
                   ...
                   <div class="refineGs">
                     ...
                     <!-- <select class="formInput" name="airportsname" id="airportsname">                      
                          ...
                          ?>       
                            ...
                            <input id="myInput" onkeyup="myFunction()" data-value="" class="bannerInputCity bannerInputCity3" name="from">
               ...
               <div class="col-sm-12">
                  <div class="table table-responsive noofdestinationsfor FlightDetail">
                    <table class="col-sm-12">                           
                      <tr>
                        <td colspan="2" class="blueColor noofdestinationsfor">Flights from <span class="source">Phnom Penh</span> to <a href="javascript:void(0)"><span class="noofdestinationsfor">0</span> destinations</a></td>
                      </tr>
                    </table>    
                  </div>
    ...
    <script>
        ...
        for (var x = 0; x < airports.length; x++) {                                    
           jQuery.getJSON('https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyCMtEH2rnetuGfmzSVLHmwkyTiipddoQec&address=' + airports[x].en + '&sensor=false', null, function (data) {               
               ...
               Loc_position[airCount] = {"lat":p.lat,"lng": p.lng,"place_Id":data.results[0].place_id,"code":airports[airCount].code};                             
               airCount++;                         
           },300);            
       }
      ...
      var marker1, marker2=new Array(20),marker3= new Array(20);
...
var destination_loc=[];
        function initMap(count) {
            ...
            document.cookie= "allLoc="+JSON.stringify(Loc_position)+";"+expires+"path=/";                
            }else{
               
                Loc_position= JSON.parse(getCookie("allLoc"));    
            }
            
            
                       
            var initialPoint={"lat":Loc_position[count].lat,"lng":Loc_position[count].lng};
            var lastPoint={"lat":Loc_position[2].lat,"lng":Loc_position[2].lng};
            var midPoint={"lat":Loc_position[5].lat,"lng":Loc_position[5].lng};
            var destination = origins[count];
           
            for(var j=0;j<destination.length;j++){                                        
               destination_loc[j]={"name":airports[destination[j][0]].en,"lat":Loc_position[destination[j][0]].lat,"lng":Loc_position[destination[j][0]].lng,"place_id":Loc_position[destination[j][0]].place_Id,"transit": destination[j][5][0][0]};    
            }
        ...
        marker1 = new google.maps.Marker({
          ...
          for(var i=0;i<destination_loc.length;i++){               
                marker2[i] = new google.maps.Marker({
                    map: map,
                    draggable: false,
                    position: {lat: destination_loc[i].lat, lng: destination_loc[i].lng},
                    icon:" "                    
                }); 
               
              //console.log(destination_loc[i].lat+" lng:"+ destination_loc[i].lng+" Code " + airports[i].code);
        ...
        var line = new google.maps.Polyline({
           path: [{lat: initialPoint.lat, lng: initialPoint.lng}, {lat: midPoint.lat, lng: midPoint.lng},{lat: lastPoint.lat,lng:lastPoint.lng}],
            geodesic: true,
            strokeColor: '#9092c2',
          icons: [{
            icon: lineSymbol,
       ...
       function update() {          
        var path=Array(20);
           for(var i=0;i<destination_loc.length;i++){    
               //console.log(marker1.getPosition(), marker2[i].getPosition());               
               if(destination_loc[i].transit!=undefined){
                   ...
                   for(var k=0;k<destination_loc.length;k++){                       
                       if(midPint==Loc_position[k].code){
                          // console.log(Loc_position[k].code+" " + k);
                           //var midPosition={"lat":Loc_position[k].lat,"lng":Loc_position[k].lng};
                           path[i]=[marker1.getPosition(), {lat:Loc_position[k].lat,lng: Loc_position[k].lng},marker2[i].getPosition()];               
                       }
                   }
               }else{                   
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCMtEH2rnetuGfmzSVLHmwkyTiipddoQec&#038;libraries=places" ></script> <!--AIzaSyCMtEH2rnetuGfmzSVLHmwkyTiipddoQec-->
